...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        dots = torch.matmul(q * self.scale, k.transpose(-1, -2) * self.scale) +1e-5
        ...
        *,
        ...
        if window_wise:
            if not channel_wise:
                ...
                self.rearrange= Rearrange('b d (x w1) (y w2) -> (b x y) (w1 w2) d', w1 = window_size, w2 = window_size)
                ...
                self.rearrange= Rearrange('b d (x w1) (y w2) -> (b x y) d (w1 w2)', w1 = window_size, w2 = window_size)
                self.expose=Rearrange('(b x y) d (w1 w2) -> b d (x w1 y w2)',w1 = window_size, w2 = window_size,x=window_rows,y=window_rows)
                self.recover= Rearrange('b d (x w1 y w2) -> b d (x w1) (y w2)',w1 = window_size, w2 = window_size,x=window_rows,y=window_rows)    
        
        elif gridlike_wise:
            if not channel_wise:
                #self.printlayer = PrintLayer("In GridLikeAttention")
                self.rearrange=Rearrange('b d (w1 x) (w2 y) -> (b x y) (w1 w2) d', w1 = window_size, w2 = window_size)
                ...
                #self.printlayer = PrintLayer("In ChannelAttention")
                self.rearrange= Rearrange('b d (w1 x) (w2 y) -> (b x y) d (w1 w2) ', w1 = window_size, w2 = window_size)
                self.expose=Rearrange('(b x y) d (w1 w2) -> b d (w1 x w2 y)',w1 = window_size, w2 = window_size,x=window_rows,y=window_rows)
                self.recover= Rearrange('b d (w1 x w2 y) -> b d (w1 x) (w2 y)',w1 = window_size, w2 = window_size,x=window_rows,y=window_rows)
