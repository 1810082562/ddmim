...
class DDMIMV9(LightningModule):
    ...
    def training_step(self, batch, batch_idx): #-> Union[int, Dict[str, Union[Tensor, Dict[str, Tensor]]]]:
        ...
        mask=F.dropout2d(mask,self.masking_ratio[0])*(1-self.masking_ratio[0])
        ...
        for i_layer,layer in enumerate(self.encoder.layers):
            ...
            H = W = int(L ** 0.5)
            if i_layer<3:
                layer_mask=rearrange(mask,"b (h r1 w r2) d -> b (h w) (d r1 r2)",h=H,w=W,r1=int(H0/H),r2=int(W0/W))
                layer_mask=repeat(layer_mask[:,:,0],"b l -> b l d",d=D)            
                #update the total mask

                # expend_mask = rearrange(layer_mask,"b (h w) d -> b d h w",h=H,w=W)
                # expend_mask=repeat(expend_mask[:,:self.hparams.channels,:,:],"b d h w-> b d (h r1) (w r2)",h=H,w=W,r1=2 ** (i_layer+2),r2=2 ** (i_layer+2))
                # total_mask=total_mask*expend_mask
                
                # masking
                x_masked=x_masked*layer_mask
        ...
        total_mask=repeat(mask,"b (h w) d -> b d (h r1) (w r2)",h=H0,w=W0,r1=4,r2=4)
        total_mask=total_mask[:,:3,:,:]
        #token_masked=x_masked[total_mask<0.5]
        token_masked=x_masked*(1-total_mask)
        #token_origin=origin[total_mask<0.5]
        token_origin=origin*(1-total_mask)
    ...
    def configure_optimizers(self):
        ...
        # lr_scheduler = StepLR(optimizer,step_size=33,gamma=0.1)
